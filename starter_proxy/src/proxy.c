 ...
 *
*/
int queue_message_send(client **clients, size_t i, char *buf, size_t msg_length) {
    size_t n = msg_length;
    size_t new_size;
...
int process_client_read(client **clients, size_t i, int data_available, fd_set *write_set) {
    ...
    if ((msg_rcvd = pop_message(&(clients[i]->recv_buf), &(clients[i]->recv_buf_len),
            &clients[i]->recv_buf_size, &msg_length)) == NULL) {
        return 0;
    }

    else {
        int sibling_idx = clients[i]->sibling_idx;
        int bytes_queued = queue_message_send(clients, sibling_idx, msg_rcvd, msg_length);
